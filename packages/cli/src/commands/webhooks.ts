...
export async function handleWebhooks(
  ...
  switch (subcommand) {
    ...
    case 'show': {
      const webhookId = positionals[0] ?? (typeof flags.webhook === 'string' ? flags.webhook : undefined);
      if (!webhookId) {
    ...
    case 'replay': {
      ...
      const toTime = typeof flags.to === 'string' ? flags.to : undefined;
    ...
    case 'replay-status': {
      const webhookId = positionals[0] ?? (typeof flags.webhook === 'string' ? flags.webhook : undefined);
      const replayId = positionals[1] ?? (typeof flags.replay === 'string' ? flags.replay : undefined);
