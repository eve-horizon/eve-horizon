...
- `./bin/eh test integration --reset-db` — **green: 203 passed / 0 failed / 13 intentionally skipped** (after fixing 3 timing-flaky tests: `harness-matrix`, `job-context`, `job-wait` raced the orchestrator for manually-driven jobs; fixed with a far-future `defer_until` at job creation — orchestrator claim queries exclude deferred jobs while manual claim/phase updates ignore `defer_until`). Note for gate runs: always use `--reset-db` — without it the shared `eve_test` DB carries stale ready-job backlogs that starve current runs and can leak an org-default harness (e.g. `zai` without `Z_AI_API_KEY`) into tests that omit `--harness`.
