...
> Purpose: Preserve MVP intent while documenting current Jobs V2 behavior.

## Current Jobs V2 Summary

- **IDs**: `{slug}-{hash8}` roots, `{parent}.{n}` children
- **Phases**: `idea → backlog → ready → active → review → done/cancelled`
- **Review**: submit/approve/reject
- **Attempts**: created on claim; logs are per attempt
- **Skills**: repo-only (`.agents/skills/`) with optional `.claude/skills/` overrides
