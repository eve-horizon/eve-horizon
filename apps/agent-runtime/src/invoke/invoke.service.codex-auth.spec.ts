...
describe('agent-runtime codex auth provenance', () => {
  ...
  it('logs api_key with the resolved secret scope when OPENAI_API_KEY is present', async () => {
    ...
    const result = await resolveCodeAuth([
      ...
      secret('CODEX_AUTH_JSON_B64', 'e30=', 'org', 'org_1'),
    ], { configDir });
    ...
    expect(lifecycle?.meta).toMatchObject({ kind: 'codex_auth_selected', source: 'api_key', secret_key: 'OPENAI_API_KEY' });
    expect(JSON.stringify(appendLog.mock.calls)).not.toContain('sk-live-secret');
  });

  it('logs auth_json when CODEX_AUTH_JSON_B64 is the highest-priority secret', async () => {
    const { appendLog, resolveCodeAuth } = makeService();
    const authJson = JSON.stringify({ tokens: { access_token: 'tok-secret', refresh_token: 'rt-secret' } });

    const result = await resolveCodeAuth([
      secret('CODEX_AUTH_JSON_B64', Buffer.from(authJson).toString('base64'), 'org', 'org_1'),
  ...
  it('logs oauth_access_token when only CODEX_OAUTH_ACCESS_TOKEN is present', async () => {
    const { appendLog, resolveCodeAuth } = makeService();
  ...
  it('logs preexisting without key or scope when a pre-existing auth.json is used', async () => {
    ...
    const result = await resolveCodeAuth([], { configDir });
  ...
  it('logs no selection when no codex credential can be found', async () => {
    ...
    await expect(resolveCodeAuth([], { configDir })).rejects.toThrow(/Missing code auth/);
