...
> ✅ **Images cut over (2026-07-22)**: `release-v0.1.314` is the first release
> published from this repo — all 7 service images are live in
> `public.ecr.aws/w7c4v0w3/eve-horizon`. Everything through `0.1.313` came from
> the retired private repo.
>
> ⚠️ **npm publishing is still blocked**: `NPM_TOKEN` is not set on this repo, so
> `cli-v*`/`sdk-v*`/`chat-v*` tags will fail. See
